...
# 7. Define pytest options (verbose output, show print statements, generate a NEW HTML report)
#pytest_options = ["-v", "-s", "--html=report.html", "--self-contained-html"]
# 7. Define pytest options (verbose output, show print statements, generate a NEW HTML report)
#pytest_options = ["-v", "-s"]
#pytest_options = ["-v", "-s", "--ff"]  # Runs only the last failed tests
#If you want to run the failed tests first, followed by the rest of the tests, you can use:
#pytest --ff

# Define pytest options to clear cache and run failed tests
